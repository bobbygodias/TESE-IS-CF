...
```mermaid
flowchart LR
    A[Substrato físico] --> B[Estados físicos]
    B --> C[Transformações de informação]
    C --> D[Organização causal]
    D --> E[Inferência]
    D --> F[Previsão]
    D --> G[Planejamento]
    D --> H[Controle]
    D --> I[Aprendizagem]
    D --> J[Generalização e robustez]

    K[Biologia específica] -. não é condição lógica necessária .-> D
```
...
```mermaid
flowchart TD
    P[Computação física] --> T[IS-CF]
    Q[Termodinâmica da informação] --> T
    R[Teoria da informação] --> T
    S[Cibernética e feedback] --> T
    U[Aprendizagem persistente] --> T

    T --> V[Cognição funcional em múltiplos substratos]
    V --> W[Previsões empiricamente testáveis]

    X[Consciência fenomenal] -. questão separada .-> T
```
...
```mermaid
flowchart LR
    A[Estado interno multidimensional] --> B[Tensões e gradientes]
    B --> C[Estado global modulador]
    C --> D[Priorização]
    C --> E[Seleção de ação]
    C --> F[Planejamento]
    C --> G[Controle]
...
```mermaid
flowchart TD
    A[Hipótese] --> B[Predição operacional]
    B --> C[Métrica observável]
    C --> D[Experimento / medição]
    D --> E{Resultado compatível?}
    E -- Sim --> F[Hipótese ganha suporte]
    E -- Não --> G[Revisão ou rejeição]
    F --> H[Novas previsões]
    G --> H
```
